...
import { APROVE_REGISTER } from '../../graphql/inscriptions/mutations.js';
import ButtonLoading from '../../components/ButtonLoading';
import { toast } from 'react-toastify';
import { AccordionStyled, AccordionSummaryStyled, AccordionDetailsStyled } from '../../components/Accordion';


const IndexInscriptions = () => {
    const { data, loading, error, refetch } = useQuery(GET_INSCRIPTIONS);
...
const AccordionInscription = ({ data, title, refetch = () => {} }) => {
    return (
        <AccordionStyled>
            ...
            <div className='flex'>
                    {data &&
                        data.map((inscription) => {
                            return <Inscription inscription={inscription} refetch={refetch}/>;
                        })}
                </div>
            </AccordionDetailsStyled>
        </AccordionStyled>
...
const Inscription = ({ inscription, refetch }) => {
    const [approveRegistration, { data, loading, error }] = useMutation(APROVE_REGISTER);
